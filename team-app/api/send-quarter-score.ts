...
// Siebte Benachrichtigungsart: Live-Ticker während des Spiels. Sobald der
// Live-Stats-Tracker (GameStatsTracker.tsx) ins nächste Viertel wechselt,
// ruft der Client die RPC announce_quarter_score auf (Migration 0042), die
// per "Ratchet" (last_announced_quarter kann nur steigen) doppelte Pushes
// beim versehentlichen Vor-/Zurückklicken verhindert. Der eigentliche
// Auslöser ist ein von Hand angelegter Trigger auf games.last_announced_quarter
// (siehe README) — diese Function bekommt nur game_id/quarter und liest den
// aktuellen Spielstand frisch aus games.final_score_us/opponent, die per
// Trigger (Migration 0028) bei jedem Stats-Event automatisch aktuell
// gehalten werden. Geht an alle mit aktivierten Push-Benachrichtigungen,
// wie "Training abgesagt"/"Kader veröffentlicht".
//
// Bewusst komplett ohne eigene lokale Imports — siehe send-training-
// reminders.ts für den Grund.

interface QuarterScorePayload {
  type?: string;
  table?: string;
  record?: {
    game_id?: string;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (queryError) {
    ...
    console.error('send-quarter-score query error', queryError);
