-- Element 09 "Spiel-Kader": ein abgesagter Spieler bleibt im Kader stehen,
-- bis der Trainer den Haken manuell entfernt — der Zähler zählt weiter die
-- nominierten Spieler mit. respond_to_squad() durfte is_selected bisher auf
-- false setzen; das entfernt den Spieler sofort aus dem Kader und
-- widerspricht der neuen Vorgabe. Nur noch confirmation/decline_reason/
-- decline_note ändern sich bei einer Absage, is_selected bleibt unberührt
-- und wird ausschließlich vom Trainer über die Kaderauswahl gesetzt.
